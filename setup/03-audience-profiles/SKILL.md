...
| "retomar", "continuar de onde parou" | Modo Retomada → ler WORKFLOW_STATE e oferecer ponto de retomada |
...
### Passo 2 — Verificar WORKFLOW_STATE

O WORKFLOW_STATE de sessão deste bloco é salvo em `[ACTIVE_COMPANY_PATH]/audiences/WORKFLOW_STATE.md` (distinto do `context/WORKFLOW_STATE.md` do pipeline de conteúdo gerado pelo orchestrator ao final do setup).

Verificar se existe `[ACTIVE_COMPANY_PATH]/audiences/WORKFLOW_STATE.md` com sessão anterior pausada.
...
PARA CADA PÚBLICO:
  ...
  [BIFURCAÇÃO B2B — verificar WORKFLOW_STATE após Skill 4.1]
  SE b2b_bifurcacao: true no WORKFLOW_STATE:
    ...
    → Ao concluir ambos: registrar no WORKFLOW_STATE ambos como completos
  SENÃO:
...
**RETOMADA:** Se o `WORKFLOW_STATE.md` indicar sessão anterior pausada, sempre oferecer retomar antes de iniciar algo novo.
...
**ATUALIZAR WORKFLOW_STATE:** Registrar ao final de cada sessão em `[ACTIVE_COMPANY_PATH]/audiences/WORKFLOW_STATE.md`:
- Empresa ativa
...
## Estrutura do WORKFLOW_STATE.md

O arquivo `WORKFLOW_STATE.md` é o "estado compartilhado" entre todas as sub-skills. Estrutura esperada:

```markdown
# WORKFLOW_STATE — [Empresa]
...
**Ver também:** `references/workflow-state-template.md` — template em branco para iniciar nova sessão.
...
- `references/workflow-state-template.md` — template em branco do WORKFLOW_STATE
