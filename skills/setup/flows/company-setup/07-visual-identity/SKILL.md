...
Consultar `skills/setup/flows/company-setup/00-orchestrator/references/startup-mode.md`, seção 3 (Bloco 7). Ajustes específicos:

- **Pergunta 7.2 (Extração de site):** se `sem_site` em `startup_flags`, pular direto e oferecer fallback estruturado:
  > "Você ainda não tem site. Vou extrair a identidade de forma estruturada. Escolha uma opção:
  > (A) 2-3 marcas-inspiração cujo visual você admira — eu analiso e sugiro paletas híbridas.
  > (B) Mood moodboard verbal — palavras-chave como: minimal, vibrante, orgânico, tech, nostálgico.
  > (C) Redes sociais próprias (se tiver Instagram/LinkedIn com posts visuais)."
- **Geração automática:** continua gerando tokens completos (variações de opacidade, paleta neutra dark/light, cores semânticas, border-radius, sombras) mesmo partindo de input mínimo (descrição verbal + 1 cor primária).
- **Output:** adicionar header `> **Status:** 🔸 Identidade visual provisória — refinar após primeiras publicações e feedback` ao topo do `VISUAL_IDENTITY.md`.
